    ...
    <section className="py-24 px-6 text-center relative overflow-hidden bg-gradient-to-br from-[#f3e7d2] via-white to-[#f3e7d2]">
      ...
      <div className="flex flex-col items-center justify-center relative max-w-xl mx-auto">
        <div className="flex items-center justify-center gap-4 mb-6">
          <button onClick={prev} aria-label="Previous" className="p-2 rounded-full bg-white shadow hover:bg-[#f3e7d2] text-[#b07a3c] text-xl transition disabled:opacity-50" disabled={testimonials.length < 2}>
            <FaChevronLeft />
          </button>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <p className="text-[#1a2341] italic mb-4 text-base md:text-lg">“{testimonials[index].quote}”</p>
                  <div className="font-semibold text-[#b07a3c]">{testimonials[index].name}</div>
                  <div className="text-[#1a2341] text-sm">{testimonials[index].company}</div>
                </div>
              </motion.div>
            </AnimatePresence>
          </div>
          <button onClick={next} aria-label="Next" className="p-2 rounded-full bg-white shadow hover:bg-[#f3e7d2] text-[#b07a3c] text-xl transition disabled:opacity-50" disabled={testimonials.length < 2}>
            <FaChevronRight />
          </button>
        </div>
        {/* Dots */}
        <div className="flex gap-2 justify-center mt-2">
          {testimonials.map((_, i) => (
            <button
              key={i}
              onClick={() => setIndex(i)}
              className={`w-3 h-3 rounded-full ${i === index ? 'bg-[#b07a3c]' : 'bg-[#e5d3b3]'} transition`}
